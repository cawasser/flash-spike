...
The `home-routes` handler in the `flash-spike.routes.home` namespace
...
The `home-page` function will in turn call the `flash-spike.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/flash_spike/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `flash-spike.handler` namespace:
...
Request middleware functions are located under the `flash-spike.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `flash-spike.dev-middleware` namespace found in
